...
def generate_code(ast, indent_level=0):
    ...
    for node in ast:
        ...
        elif node[0] == "book_reservation":
            generated_code += handle_book_reservation(node, indent_level)
        elif node[0] == "book_reservation_no_location":
            generated_code += handle_book_reservation_no_loc(node, indent_level)
        elif node[0] == "cancel_reservation":
            generated_code += handle_cancel_reservation(node, indent_level)
        elif node[0] == "cancel_reservation_with_identifier":
            generated_code += handle_cancel_reservation_identifier(node, indent_level)
        elif node[0] == "confirm_reservation":
            generated_code += handle_confirm_reservation(node, indent_level)
        elif node[0] == "confirm_reservation_with_identifier":
            generated_code += handle_confirm_reservation_identifier(node, indent_level)
        elif node[0] == "pay_reservation":
            generated_code += handle_pay_reservation(node, indent_level)
        elif node[0] == "pay_reservation_with_identifier":
            generated_code += handle_pay_reservation_identifier(node, indent_level)
        elif node[0] == "view_reservation_with_status":
            generated_code += handle_view_reservations_with_status(node, indent_level)
        elif node[0] == "view_reservation_with_name":
            generated_code += handle_view_reservations_specific(node, indent_level)
        elif node[0] == "view_openai_response":
            generated_code += handle_view_openai_response(node, indent_level)
        elif node[0] == "book_ticket":
            generated_code += handle_book_ticket(node, indent_level)
        elif node[0] == "cancel_ticket":
            generated_code += handle_cancel_ticket(node, indent_level)
        elif node[0] == "cancel_ticket_with_identifier":
            generated_code += handle_cancel_ticket_identifier(node, indent_level)
        elif node[0] == "view_tickets_with_status":
            generated_code += handle_view_tickets_status(node, indent_level)
        elif node[0] == "view_specific_tickets":
            generated_code += handle_view_tickets_specific(node, indent_level)
        else:
...
def handle_book_reservation(node, indent_level):
    indent = "    " * indent_level
    return f"{indent}handler.book_reservation('{node[1]}','{node[2]}','{node[3]}','{node[4]}','{node[5]}','{node[6]}')\n"


def handle_book_reservation_no_loc(node, indent_level):
    indent = "    " * indent_level
    return f"{indent}handler.book_reservation('{node[1]}','{node[2]}','{node[3]}', None, None,'{node[4]}')\n"


def handle_book_ticket(node, indent_level):
    indent = "    " * indent_level
    return f"{indent}handler.book_ticket('{node[1]}','{node[2]}','{node[3]}','{node[4]}','{node[5]}','{node[6]}')\n"


def handle_cancel_reservation(node, indent_level):
    indent = "    " * indent_level
    return f"{indent}handler.cancel_reservation('{node[1]}','{node[2]}')\n"


def handle_cancel_ticket(node, indent_level):
    indent = "    " * indent_level
    return f"{indent}handler.cancel_ticket('{node[1]}','{node[2]}')\n"


def handle_cancel_reservation_identifier(node, indent_level):
    indent = "    " * indent_level
    return f"{indent}handler.cancel_reservation_identifier('{node[1]}','{node[2]}','{node[3]}')\n"


def handle_cancel_ticket_identifier(node, indent_level):
    indent = "    " * indent_level
    return f"{indent}handler.cancel_ticket_identifier('{node[1]}','{node[2]}','{node[3]}')\n"


def handle_confirm_reservation(node, indent_level):
    indent = "    " * indent_level
    return f"{indent}handler.confirm_reservation('{node[1]}','{node[2]}')\n"


def handle_confirm_reservation_identifier(node, indent_level):
    indent = "    " * indent_level
    return f"{indent}handler.confirm_reservation_identifier('{node[1]}','{node[2]}','{node[3]}')\n"


def handle_pay_reservation(node, indent_level):
    indent = "    " * indent_level
    return f"{indent}handler.pay_reservation('{node[1]}','{node[2]}')\n"


def handle_pay_reservation_identifier(node, indent_level):
    indent = "    " * indent_level
    return f"{indent}handler.pay_reservation_identifier('{node[1]}','{node[2]}','{node[3]}')\n"


def handle_view_reservations_with_status(node, indent_level):
    indent = "    " * indent_level
    if (node[1].upper()) == "PAID":
        return f"{indent}handler.view_list_payed_reservations()\n"
    else:
        return f"{indent}handler.view_list_reservations_with_status('{node[1]}')\n"


def handle_view_reservations_specific(node, indent_level):
    indent = "    " * indent_level
    return f"{indent}handler.view_list_reservations_specific('{node[1]}')\n"


def handle_view_tickets_specific(node, indent_level):
    indent = "    " * indent_level
    return f"{indent}handler.view_list_tickets_specific('{node[1]}')\n"


def handle_view_tickets_status(node, indent_level):
    indent = "    " * indent_level
    return f"{indent}handler.view_list_tickets_status('{node[1]}')\n"


def handle_view_openai_response(node, indent_level):
    indent = "    " * indent_level
    return f"{indent}handler.view_api_response('{node[1]}')\n"
...
def handle_conditionals(node, indent_level):
    if node[1][0] == "if":
        ...
        return handle_repeatif(node[1], indent_level)
...
# Generates the code for the while loop
def handle_repeatif(node, indent_level):
    indent = "    " * indent_level
    length = len(node[1])
    repeatif = node[2]
    generated_code = ""
